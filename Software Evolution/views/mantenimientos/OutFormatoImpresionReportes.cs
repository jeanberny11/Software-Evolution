...
{
    public partial class OutFormatoImpresionReportes : BaseForm
    {
        FormatoImpresionReportesManager manager = new FormatoImpresionReportesManager();
        public OutFormatoImpresionReportes()
        {
            InitializeComponent();
            Buscar();
        }
        ...
        {
            var form = new InFormatoImpresionReportes();
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (e.Clicks == 2)
            {
                var id = Convert.ToInt32(gridView1.GetRowCellValue(e.RowHandle, "f_id"));
                var form = new InFormatoImpresionReportes(id);
                (this.ParentForm as Principal).ShowOrFocusForm(form);
            }
        }
    }
